...
export function getConfigFilePath(client: string): string {
  switch (client.toLowerCase()) {
    ...
    case 'claude-code':
      return '.mcp.json (project) or ~/.claude.json (global)';
    case 'cursor':
    ...
    case 'vscode':
      return '.vscode/mcp.json (workspace) or User Profile mcp.json (global)';
    case 'codex':
      return '~/.codex/config.toml (global) or .codex/config.toml (project)';
    case 'gemini':
      return '~/.gemini/settings.json (global) or .gemini/settings.json (project)';
    case 'goose':
      return '~/.config/goose/config.yaml';
    case 'continue':
      return '.continue/mcpServers/*.yaml (workspace) or ~/.continue/config.yaml (global)';
    case 'zed':
      return '~/.config/zed/settings.json (macOS/Linux)\n  %APPDATA%\\Zed\\settings.json (Windows)';
    case 'amp':
      return '~/.config/amp/settings.json';
    case 'jetbrains':
      return 'IDE Settings → Tools → AI Assistant → MCP';
    default:
      return '';
  }
}
